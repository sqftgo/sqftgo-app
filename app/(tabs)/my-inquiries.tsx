...
const STATUS_STYLE: Record<InquiryStatus, { bg: string; color: string; label: string }> = {
  open: { bg: colors.infoSoft, color: colors.info, label: "Open" },
  replied: { bg: colors.successSoft, color: colors.success, label: "Replied" },
  dismissed: { bg: colors.surfaceSubtle, color: colors.inkMuted, label: "Dismissed" },
};
...
export default function MyInquiriesScreen() {
  ...
  const mine = useMemo(
    () => inquiries.filter((i) => i.buyerEmail === userEmail),
    [inquiries, userEmail],
  );
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            message="Open a listing and submit an inquiry to contact the dealer."
